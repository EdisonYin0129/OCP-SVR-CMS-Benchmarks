...
def cleanup():
    ...
    container_exists = subprocess.run(
        ["docker", "ps", "-a", "-q", "-f", "name=qdrant"], stdout=subprocess.PIPE
    ).stdout
    if container_exists:
        # Check if the container was killed due to an OOM error
        inspect_output = subprocess.run(
            ["docker", "inspect", "qdrant"], stdout=subprocess.PIPE
        ).stdout
        if b'"OOMKilled": true' in inspect_output:
            logger.error(
                "Qdrant container was killed due to an Out of Memory (OOM) error."
            )
        else:
...
def run_qdrant_container(
    cpus, memory, dbpath, storage, port, numa_nodes=None, cpu_set=None
):
    logger.info(
        f"Starting Qdrant container with {cpus} CPUs, {memory}GB memory, {storage}GB storage, and port {port}..."
    )
    cmd = [
        "docker",
        "run",
        ...
        f"{memory}g",
        "--mount",
        f"type=bind,src={dbpath},dst=/qdrant/storage,readonly=false",
...
def create_collection(
    ...
    logger.info(
        f"Creating collection '{collection_name}' with vector size {vector_size} using datatype {qdrant_data_type}..."
    )
...
def insert_vectors(
    ...
    if disable_indexing_for_loading:
        logger.info(
            f"Disabling HNSW indexing for collection '{collection_name}' during vector insertion..."
        )
        client.update_collection(
            collection_name=collection_name,
            hnsw_config=models.HnswConfigDiff(m=0),  # Temporarily disable HNSW indexing
        )
    ...
    if data_type == "float32":
        ...
        vectors = np.random.randint(
            0, 256, size=(num_vectors, vector_size), dtype=np.uint8
        )
    else:
        raise ValueError(f"Unsupported data type: {data_type}")

    logger.info(
        f"Inserting {num_vectors} vectors into collection '{collection_name}' using {data_type} datatype..."
    )
    inserted_count = 0
    insertion_rates = []
    start_time = time.time()
    ...
    with tqdm(total=num_vectors, desc="Inserting vectors", unit="vectors") as pbar:
        for i in range(0, num_vectors, batch_size):
            ...
            insertion_rate = (
                current_batch_size / batch_duration if batch_duration > 0 else 0
            )
    ...
    logger.info(
        f"Inserted {inserted_count} vectors into collection '{collection_name}'."
    )
...
def measure_performance(
    client, collection_name, vector_size, data_type, num_queries=1000
):
    ...
    logger.info(
        f"Average query time: {avg_query_time:.6f} seconds (over {actual_queries} queries)"
    )
...
def format_duration(seconds):
    ...
    if (
        seconds > 0 or not formatted_time
    ):  # Always show seconds if nothing else is shown
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark Qdrant for performance testing by inserting a specified number of vectors. The tool helps measure Qdrant performance under different configurations, including CPU, memory, storage, and vector size."
    )
    ...
    parser.add_argument(
        "--numa-nodes", type=str, default=0, help='NUMA nodes to use (e.g., "0,1")'
    )
    parser.add_argument(
        "--cpu-set",
        type=str,
        help='Specific CPUs or CPU sockets to use (e.g., "0-3,4-7" or "0,1")',
    )
    parser.add_argument(
        "--vector-size", type=int, default=384, help="Vector size for the collection"
    )
    parser.add_argument(
        "--numvectors",
        type=int,
        default=1000000,
        help="Number of vectors to insert (must be a positive integer)",
    )
    parser.add_argument(
        "--data-type",
        type=str,
        default="FP32",
        choices=["FP32", "UINT8"],
        help="Data type for vectors (FP32, UINT8)",
    )
    parser.add_argument(
        "--dbpath",
        type=str,
        default="/tmp/qdrant/storage",
        help="Directory on the host to hold Qdrant collection files",
    )
    parser.add_argument(
        "--on-disk",
        action="store_true",
        help="Enable memory-mapped storage for vectors",
    )
    parser.add_argument(
        "--hnsw-on-disk",
        action="store_true",
        help="Enable on-disk storage for HNSW index",
    )
    parser.add_argument(
        "--on-disk-payload",
        action="store_true",
        help="Enable on-disk storage for payloads",
    )
    parser.add_argument(
        "--disable-hnsw-indexing-for-loading",
        action="store_true",
        help="Disable HNSW indexing during vector loading and re-enable it afterward",
    )
    parser.add_argument(
        "--batch-size", type=int, default=1000, help="Number of vectors per batch"
    )
    parser.add_argument(
    ...
    if not any(flag in sys.argv for flag in help_flags):
        ...
        )  # Use pprint to nicely format the dictionary
        ...
        try:
            ...
            logger.info("Disk usage before initial insertion:")
            ...
            logger.info(
                f"Successfully inserted {args.numvectors} vectors in {formatted_duration} ({insertion_duration:.2f} seconds)."
            )

            db_size_bytes = get_database_size(client, collection_name)
            logger.info(
                f"Database size after initial insertion: {format_size(db_size_bytes)}"
            )
            ...
            avg_query_time = measure_performance(
                client, collection_name, args.vector_size, args.data_type
            )
            ...
            logger.info(
                f"Total benchmark completed in {formatted_duration} ({benchmark_duration:.2f} seconds)."
            )

            # Report overall insertion rates
            logger.info("Overall Insertion Rate Summary:")
            logger.info(
                f"Initial insertion - Avg: {insert_avg:.2f}, Min: {insert_min:.2f}, Max: {insert_max:.2f} vectors/second"
            )
